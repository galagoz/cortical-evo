#----------------------------------------------------------------------------------------------------------
# This script uses the ouptut created by begenie_merge.py to create manhatten and qq plots for all different samples (replication/european, surface,thickness etc)
...
for (sample_set in c('replication','european'))
  {
  setwd(paste0('/data/workspaces/lag/workspaces/lg-ukbiobank/projects/enigma_evol/final/',sample_set,'/bgenieOutput/global/'))
  if (sample_set == 'replication')
    {
    for (pheno in c('surface', 'thickness'))
      ...
      gwas = fread(paste0('sumstats_ukb43760_global_',pheno,'_',sample_set,'_allChr.gz'), header = T, select= c(1,7,10,11))
      # get the phenotype name from the column header
      cur_pheno = paste0('ukb43769_global_',pheno,'_',sample_set)
      # create a plot title
      #save the png
    ...
    write.csv(final_lambda,file=paste('bgenie_ukb43760_global_',cur_pheno,'_lambdas_chisqStats.csv',sep=""),row.names = FALSE)
    lambdalist=list()
  }
  else
    {
    for (pheno in c('surface', 'thickness'))
      ...
      for (hemi in c('le','re'))
        {
        # only load stuff we need which is Chr, pos, rsid and one phenotype --> NOTE they have been converted back to pvalues during output merge
        gwas = fread(paste0('sumstats_ukb43760_global_',pheno,'_',hemi,'_',sample_set,'_allChr.gz'), header = T, select= c(1,7,10,11))
        # get the phenotype name from the column header
        cur_pheno = paste0('ukb43769_global_',pheno,'_',hemi,'_',sample_set)
        # create a plot title
        #save the png
        png(paste0('plots/bgenie_manhattan_',cur_pheno,'_.png'),width = 1000, type = "cairo")
        #create the manhattan with certain settings
