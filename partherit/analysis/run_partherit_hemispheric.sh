...
while read a; do
   ...
   echo $tmp_file
   tmp_pheno="$(cut -d'_' -f5,6- <<<"$tmp_file")" #TODO find a better way to do this
   echo $tmp_pheno
...
/data/clusterfs/lag/users/gokala/enigma-evol/final_analysis/scripts/cortical-evo/partherit/analysis/partherit_baseline_hemispheric.sh '$a' '$annot' '$tmp_output'' > $shellFile
...
/data/clusterfs/lag/users/gokala/enigma-evol/final_analysis/scripts/cortical-evo/partherit/analysis/partherit_extraAnnot_hemispheric.sh '$a' '$annot' '$tmp_output' '$baseline'' > $shellFile
